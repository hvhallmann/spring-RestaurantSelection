...
public class JpaRestaurantRepositoryImpl implements RestaurantRepository {
    ...
    public Collection<Restaurant> findAll() {
        ...
        return this.em.createQuery("SELECT distinct restaurant.mainName FROM Restaurant restaurant").getResultList();
    }
    
    @Override
    //public Restaurant findIdByMainName(String sLastName) throws DataAccessException;
    public Restaurant findIdByMainName(String sLastName) throws DataAccessException
    {
        // using 'join fetch' because a single query should load both owners and pets
        // using 'left join fetch' because it might happen that an owner does not have pets yet
        Query query = this.em.createQuery("SELECT restaurant FROM Restaurant restaurant WHERE restaurant.mainName =:lastName");
        query.setParameter("lastName", sLastName);
